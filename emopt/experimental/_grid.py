...
def intersection(k, list_shapes, nl=nl_sig):
    # this is the main differentiable union function
    return torch.clamp(sum(list_shapes), min=1.0, max=2.0) - 1.0

def intersection_b(k, list_shapes, nl=nl_sig):
    # only works for two shapes, need to improve
    return math.prod(list_shapes)

def intersection_c(k, list_shapes, nl=nl_sig):
    # legacy intersection
    return nl(k, sum(list_shapes) - 1.5)
