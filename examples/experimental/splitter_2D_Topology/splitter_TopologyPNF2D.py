...
from emopt.experimental.adjoint_method import TopologyPNF2D, Topology
...
#class TopologyAM(Topology):
    def __init__(self, sim, mode_match, mm_line, domain=None, update_mu=False, eps_bounds=None, mu_bounds=None, planar=False, lam=0):
        ...
        #domain_size = self._domain.Nx * self._domain.Ny
        #self.MMM =  self._domain.Nx * self._domain.Ny
    ...
    #def calc_dFdx(self, sim, params):
        ...
        #dFdEz, dFdHx, dFdHy = emopt.fomutils.interpolated_dFdx_2D(sim, dFdEz, dFdHx, dFdHy)

        return (dFdEz, dFdHx, dFdHy)
        #return (dFdEz*1e3, dFdHx*1e3, dFdHy*1e3)
...
if __name__ == '__main__':
    ...
    #inrect = emopt.misc.DomainCoordinates(0.0, 5.0, 0.5*Y-0.25, 0.5*Y+0.25, 0.0, 0.0, dx, dy, 1.0)
    #outrect1 = emopt.misc.DomainCoordinates(X-5.0, X+5.0, 0.5*Y-0.25+1, 0.5*Y+0.25+1, 0.0, 0.0, dx, dy, 1.0)
    #outrect2 = emopt.misc.DomainCoordinates(X-5.0, X+5.0, 0.5*Y-0.25-1, 0.5*Y+0.25-1, 0.0, 0.0, dx, dy, 1.0)

    #grid_eps = eps_clad*np.ones((M, N))
    #grid_eps[optdomain.j, optdomain.k] = eps_si
    #grid_eps[inrect.j, inrect.k] = eps_si
    #grid_eps[outrect1.j, outrect1.k] = eps_si
    #grid_eps[outrect2.j, outrect2.k] = eps_si
    #eps = emopt.grid.GridMaterial2D(N, M, grid_eps)
    ...
    #am.check_gradient(design_params, indices=[1, 40, 100], fd_step=1e-8)
    #am.check_gradient(design_params, indices=[1, 40, 100], fd_step=1e-3)
    #am.check_gradient(design_params, indices=np.arange(100)[::2], fd_step=1e-4)
    #am.check_gradient(design_params, indices=np.arange(100)[::2], fd_step=1e-6)
    am.check_gradient(design_params, indices=np.arange(100)[::2], fd_step=1e-4)
