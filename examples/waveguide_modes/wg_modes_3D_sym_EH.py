"""Solve for the fundamental TM-like mode using electric and magnetic symmetry.

On most *nix-based machines, run the script with:

    $ mpirun -n 8 python wg_modes_3D_sym_EH.py
...
strip = emopt.grid.Rectangle(0.0, 0.0, w_wg, h_SOI)
...
Ex = modes.get_field_interp(0, 'Ey', squeeze=True)
if(NOT_PARALLEL):
